# Published Dynamic-Behavior Asset Alignment

This file maps the uploaded published dynamic-behavior PDF tables and figures to the evidence currently available in this checkout.
...
| Asset | Published label | Status | Current source | Caveat |
| --- | --- | --- | --- | --- |
...
| Figure 1 | Framework overview | current_data_bridge | fig1_study_pipeline.{png,pdf,svg} | It is a current-method figure, not the original published figure. |
...
| Table III | Phase-level distribution and dispersion diagnostics from per-app mean RDI | current_data_bridge | baseline_interactive_metric_summary.csv | Do not report published RDI mean, SD, CV, Shapiro, or variance-ratio values unless original-compatible RDI rows are regenerated. |
| Table IV | Paired effect size for interaction-induced deviation shifts | current_data_bridge | baseline_interactive_metric_summary.csv | Cohen's dz requires paired per-app RDI deltas. |
| Table V | Descriptive statistics of per-app deviation shifts | current_data_bridge | baseline_interactive_metric_summary.csv | Do not copy the original n, mean, median, IQR, Wilcoxon, or n+/n- RDI values without the original-compatible RDI table. |
...
| Figure 2 | Per-app deviation shifts | current_data_bridge | baseline_interactive_metric_summary.csv | Delta-D bar/point plotting requires per-app RDI deltas. |
...
The current checkout can produce a dynamic-behavior method bridge, but it should not claim regenerated original RDI tables unless the frozen RDI prevalence and static-deviation tables are restored or regenerated.
